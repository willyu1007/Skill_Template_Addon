---
name: feature-db-mirror
description: Enable and operate the DB Mirror feature (db/ schema snapshots + migration workdocs) for projects where the real database is the schema SSOT.
category: features/db-mirror
ssot_path: .ai/skills/features/db-mirror/feature-db-mirror
---

# feature-db-mirror (entry)

Canonical source: `.ai/skills/features/db-mirror/feature-db-mirror/`

Open `.ai/skills/features/db-mirror/feature-db-mirror/SKILL.md` and any supporting files referenced there (for example `reference.md`, `examples.md`, `scripts/`, `templates/`).

> **Note**: The frontmatter above is identical to the canonical source except for `ssot_path` and `category` which are added for navigation. After opening the source file, skip re-reading the description to avoid redundant token usage.
