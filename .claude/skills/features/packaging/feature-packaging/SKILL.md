---
name: feature-packaging
description: Enable and operate the Packaging feature (ops/packaging conventions + packctl) for building runnable artifacts (usually container images).
category: features/packaging
ssot_path: .ai/skills/features/packaging/feature-packaging
---

# feature-packaging (entry)

Canonical source: `.ai/skills/features/packaging/feature-packaging/`

Open `.ai/skills/features/packaging/feature-packaging/SKILL.md` and any supporting files referenced there (for example `reference.md`, `examples.md`, `scripts/`, `templates/`).

> **Note**: The frontmatter above is identical to the canonical source except for `ssot_path` and `category` which are added for navigation. After opening the source file, skip re-reading the description to avoid redundant token usage.
