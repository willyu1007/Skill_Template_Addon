...
During the Stage C completion checkpoint (before approving Stage C), ensure `init/skill-retention-table.template.md` exists (generated by the pipeline). Fill the table with skills from `.ai/skills/` and translate the Description column if needed. Ask the user which skills to keep/delete (record TBD if undecided).

List deletions under `## Deletion List` in `init/skill-retention-table.template.md`, then confirm deletions **before** running:

```bash
node init/skills/initialize-project-from-requirements/scripts/init-pipeline.mjs skill-retention --repo-root .
```

After confirmation, re-run with `--apply` to delete (this calls `.ai/scripts/sync-skills.mjs` under the hood):
...
At Stage C completion checkpoint, present option: "update root docs" to record tech stack in root `AGENTS.md` and refresh root `README.md`.

### README.md update

- Use `init/project-blueprint.json` as the source of truth.
- Prefer `update-root-docs` (shows a diff; requires `--apply` to write).
...
**Option B: Archive all to `docs/project/overview/` + remove `init/`** (recommended; keeps SSOT files in `docs/project/` clean)
...
The command archives Stage A docs + blueprint + init state from `init/` to `docs/project/overview/` before removing `init/`.


**Selective archive options:**
- `--archive` - Archive all (docs + blueprint + init state)
- `--archive-docs` - Archive Stage A docs only
- `--archive-blueprint` - Archive blueprint only
- `--archive-state` - Archive `init/.init-state.json` only
- `--archive-dir <path>` - Override archive destination directory (default: `docs/project/overview/`)
- `--overwrite` - Overwrite archived files (default: copy-if-missing)
