...
> After initialization completes, use `cleanup-init --archive` to move these files to `docs/project/overview/` for long-term retention.
...
Stage C `apply` does **not** auto-update the root `README.md` / `AGENTS.md`. Use `update-root-docs` to preview a diff and (after explicit approval) apply the updates; include those changes in the Stage C review if you run it before approval.
