 ...
 * deployctl.js
 *
 * Deployment configuration + planning controller for the deployment add-on.
 *
 * Safety model:
...
Examples:
  ...
  node .ai/scripts/deployctl.js add-service --id api --artifact ghcr.io/acme/api:v1.2.3
...
- This add-on plans deployments; humans execute the printed commands.
...
function cmdListServices(repoRoot, format) {
  ...
  if (config.services.length === 0) {
    ...
    console.log('  Run: node .ai/scripts/deployctl.js add-service --id api --artifact ghcr.io/acme/api:v1.2.3');
    return;
  }
